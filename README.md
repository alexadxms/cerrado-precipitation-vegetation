...
- **"Precipitation" in the correlation/trend analysis means monthly total rainfall**, not a separate extreme-event-frequency index. The original presentation also looked at extreme-day frequency (P95 threshold) separately, which this rebuild doesn't reproduce.
- **NDVI is computed from MOD09GA surface reflectance**, not the standard pre-built MOD13Q1 NDVI product. This is less quality-controlled (no built-in cloud/QA masking).
...
The biome's vegetation relies on stored soil moisture rather than reacting to rainfall immediately, so the strongest positive vegetation response appears 3-4 months after the rain that triggered it.
...
January stands out: precipitation is declining there while NDVI is increasing, a vegetation adaptation or land-use signal rather than a simple rainfall-driven response. June-August show a comparatively flat precipitation trend (dry season), suggesting NDVI decline in that period reflects degradation/fire activity rather than rainfall change.
